...
while [  ${COUNTER} -lt ${NO_OF_PEERS} ]; do
	...
	# Register the peer
	fabric-ca-client register -d --id.name ${PEER}.{{ item.external_url_suffix }} --id.secret ${PEER}-pw --id.type peer --tls.certfiles ${ROOT_TLS_CERT} --home ${CAS_FOLDER} -u https://${CA}

	# Enroll to get peers TLS cert
	fabric-ca-client enroll -d --enrollment.profile tls -u https://${PEER}.{{ item.external_url_suffix }}:${PEER}-pw@${CA} -M ${ORG_HOME}/cas/peers/tls --csr.hosts "${CSR_HOSTS}" --tls.certfiles ${ROOT_TLS_CERT}  --csr.names "${SUBJECT_PEER}"
	...
	fabric-ca-client enroll -d -u https://${PEER}.{{ item.external_url_suffix }}:${PEER}-pw@${CA} -M ${ORG_CYPTO_FOLDER}/peers/${PEER}/msp --tls.certfiles ${ROOT_TLS_CERT}  --csr.names "${SUBJECT_PEER}"
